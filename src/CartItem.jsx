...
const CartItem = ({ onContinueShopping }) => {
  ...
  const calculateTotalAmount = () => {
    ...
    cart.forEach((item) => {
      // Extract quantity and cost from each item
  ...
  const handleContinueShopping = (e) => {
    e.preventDefault();
    // Call the function passed from parent component to return to product listing
  ...
  const handleDecrement = (item) => {
    ...
    if (item.quantity > 1) {
      // If yes, dispatch updateQuantity to decrease quantity by 1
      dispatch(updateQuantity({ 
        name: item.name, 
        quantity: item.quantity - 1 
      }));
